...
def np_puzzle_to_string(np_puzzle: np.ndarray) -> str:
    ...
    if np_puzzle.shape != SHAPE_3D:
        raise ValueError(f'Expected puzzle shape {SHAPE_3D}, but got {np_puzzle.shape}.')
...
def generate_cell_index_updates(*iterables: Iterable[int]) -> Generator[Tuple[Tuple[None, int], ...], None, None]:
